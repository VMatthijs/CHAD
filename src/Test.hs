...
instance KnownNat n => FinDiff (Vect n) where
    ...
    oneHotVecs primal =
        [V.generate (\k -> if k == i then itemvec else zero)
        | i <- [minBound..maxBound]
        , itemvec <- oneHotVecs (primal `V.index` i)]

    rebuild' primal l =
        let result = mapAccumL (\l' prim -> swap (rebuild' prim l'))
                               l (V.toList primal)
...
data SomeVect = forall n. SomeVect (Vect n)
...
class Show a => Approx a where
    ...
    x `isApproxQC` y =
        ...
        in counterexample (unlines ["Control: " ++ show x
                                   ,"AD:      " ++ show y
                                   ,"Errors:  " ++ errs])
                          ok

instance Approx Scal where
    x `isApprox'` y = let err = relAbsDiff x y in (err < 0.001, show err)
...
propAll :: (Arbitrary a, Arbitrary b, Show a
           ...
           ,Element a ~ Scal
           ,a ~ Df1 a, a ~ Df2 a, b ~ Df1 b, b ~ Df2 b
           ,a ~ Dr1 a, a ~ Dr2 a, b ~ Dr1 b, b ~ Dr2 b)
        => SL.STerm a b -> Property
...
propAll' :: (Arbitrary a, Arbitrary b, Show a
            ...
            ,a ~ Df1 a, a ~ Df2 a, b ~ Df1 b, b ~ Df2 b
            ,a ~ Dr1 a, a ~ Dr2 a, b ~ Dr1 b, b ~ Dr2 b)
         => Gen a -> (a -> Gen a) -> (b -> Gen b) -> (a -> Property -> Property) -> SL.STerm a b -> Property
propAll' arggen dirgen adjgen propfun sterm = conjoin
    [counterexample "testing: fwd1"
        (forAll arggen $ \arg -> propfun arg $                       propFwd1 sterm arg)
    ,counterexample "testing: fwd2"
        (forAll arggen $ \arg -> propfun arg $ forAll (dirgen arg) $ propFwd2 sterm arg)
    ,counterexample "testing: rev1"
        (forAll arggen $ \arg -> propfun arg $                       propRev1 sterm arg)
    ,counterexample "testing: rev2"
        (forAll arggen $ \arg -> propfun arg $                       propRev2 sterm arg adjgen)
    ]

propFwd1 :: (Show a, Approx b, a ~ Df1 a, b ~ Df1 b)
         => SL.STerm a b -> a -> Property
propFwd1 sterm arg =
...
propRev1 :: (Approx b, a ~ Dr1 a, b ~ Dr1 b)
         => SL.STerm a b -> a -> Property
propRev1 sterm arg =
    let resEval = SL.evalSt sterm arg
...
propRev2 :: (FinDiff a, FinDiff b, Element a ~ Element b, Approx a, Show b, a ~ Dr1 a, a ~ Dr2 a, b ~ Dr2 b)
...
propFwdVsRev :: (Arbitrary a, Arbitrary b, Show b, FinDiff a, FinDiff b, Element a ~ Element b, Approx a, a ~ Df1 a, a ~ Df2 a, a ~ Dr1 a, a ~ Dr2 a, b ~ Df2 b, b ~ Dr2 b)
             => SL.STerm a b -> Property
propFwdVsRev = propFwdVsRev' arbitrary arbitrary

propFwdVsRev' :: (Arbitrary a, Arbitrary b, Show b, FinDiff a, FinDiff b, Element a ~ Element b, Approx a, a ~ Df1 a, a ~ Df2 a, a ~ Dr1 a, a ~ Dr2 a, b ~ Df2 b, b ~ Dr2 b)
              => Gen a -> Gen b -> SL.STerm a b -> Property
propFwdVsRev' arggen adjgen sterm = forAll arggen $ \arg -> forAll adjgen $ \adjoint ->
    let fdvals = [dotprod adjoint (lApp (TL.evalTt (F.d2 sterm) arg) v) | v <- oneHotVecs arg]
        resFD = rebuild arg fdvals
        resAD = lApp (TL.evalTt (R.d2 sterm) arg) adjoint
    in resFD `isApproxQC` resAD
...
slowTests :: TestTree
slowTests = testGroup "Slow"
    [testProperty "fact"
        (propAll' (choose (-2, 20) `suchThat` (\x -> x - fromIntegral (floor x :: Int) > 0.1))
                  (\_ -> choose (0.1, 0.5)) (\_ -> choose (0.1, 0.5))
                  (const id)
                  E.fact)
    ,testProperty "factExtra"
        (propAll' ((,) <$> (choose (-2, 20) `suchThat` (\x -> x /= floorf x)) <*> arbitrarySizedFractional)
                  (\_ -> (,) <$> choose (0.1, 0.5) <*> arbitrarySizedFractional)
                  (\_ -> choose (0.1, 0.5))
                  (const id)
                  E.factExtra)
    ,testProperty "fact2"
        (propAll' (choose (-2, 5) `suchThat` (\x -> x /= floorf x))
                  (\_ -> choose (0.1, 0.5)) (\_ -> choose (0.1, 0.5))
                  (const id)
                  E.fact2)
    ,testProperty "fact2Extra"
        (propAll' ((,) <$> (choose (-2, 5) `suchThat` (\x -> x /= floorf x)) <*> arbitrarySizedFractional)
                  (\_ -> (,) <$> choose (0.1, 0.5) <*> arbitrarySizedFractional)
                  (\_ -> choose (0.1, 0.5))
                  (const id)
                  E.fact2Extra)
