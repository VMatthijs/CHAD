...
d1 SL.Map =
  TL.Lambda $
    TL.Map (TL.Lambda $ TL.Fst (TL.Fst (TL.Var (S Z)) `TL.App` TL.Var Z))
           (TL.Snd (TL.Var Z))
...
d2 SL.Inl = TL.Lambda $ TL.LCoPair TL.LId TL.Zero
d2 SL.Inr = TL.Lambda $ TL.LCoPair TL.Zero TL.LId
d2 (SL.CoPair f g) =
  TL.Lambda $
    TL.Case
      (TL.Var Z)
