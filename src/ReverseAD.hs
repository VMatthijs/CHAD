...
d1 :: (LT a, LT b) => SL.STerm a b -> TL.TTerm (Dr1 a -> Dr1 b)
...
d2 :: (LT a, LT b) => SL.STerm a b -> TL.TTerm (Dr1 a -> LFun (Dr2 b) (Dr2 a))
...
-- f(x, y) = x + y -> [[1, 1, ...], [1, 1, ...]]
d2 (SL.Op EAdd   )      = TL.Lambda "_" inferType
                        $ TL.Lambda "x" t $ TL.LApp (TL.LOp DEAddT) (TL.Var "x" t)
    where t = inferType
d2 (SL.Op EProd  )      = undefined -- f(x, y) = x * y -> [y, x]
